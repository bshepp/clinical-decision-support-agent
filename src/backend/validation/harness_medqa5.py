"""
MedQA-5 (5-option) dataset fetcher and validation harness.

Same USMLE questions as MedQA but with 5 answer options (A-E) instead of 4,
making it a harder benchmark. Evaluates the CDS pipeline's diagnostic accuracy
against a more challenging multiple-choice format.

Source: https://huggingface.co/datasets/GBaker/MedQA-USMLE-4-options (5-opt variant)
Format: JSONL with {question, options: {A, B, C, D, E}, answer_idx, answer}

Metrics:
  - mcq_accuracy: LLM selects correct MCQ option (A-E)
...
# HuggingFace Datasets API for 5-option MedQA
MEDQA5_HF_URL = "https://datasets-server.huggingface.co/rows?dataset=GBaker/MedQA-USMLE-4-options-hf&config=default&split=test&offset={offset}&length={length}"

# Alternative: direct JSONL (5-option format)
MEDQA5_JSONL_URL = "https://huggingface.co/datasets/GBaker/MedQA-USMLE-4-options/resolve/main/phrases_no_exclude_test.jsonl"


async def fetch_medqa5(max_cases: int = 50, seed: int = 42) -> List[ValidationCase]:
    """
    Download MedQA 5-option test set and convert to ValidationCase objects.

    The 5-option format adds one additional distractor per question,
    making MCQ selection harder (20% chance vs 25% random baseline).
    ...
    """
    ensure_data_dir()
    cache_path = DATA_DIR / "medqa5_test.json"

    if cache_path.exists():
        print(f"  Loading MedQA-5 from cache: {cache_path}")
        raw_cases = json.loads(cache_path.read_text(encoding="utf-8"))
    else:
        print(f"  Downloading MedQA 5-option test set...")
        raw_cases = await _download_medqa5(cache_path)

    if not raw_cases:
        raise RuntimeError("Failed to fetch MedQA-5 data. Check network connection.")
    ...
    for i, item in enumerate(raw_cases):
        ...
        case_obj = ValidationCase(
            ...
            ground_truth={
                ...
                "n_options": len(options) if isinstance(options, dict) else 4,
            },
            metadata={
    ...
    print(f"  Loaded {len(cases)} MedQA-5 cases")
    return cases
...
async def _download_medqa5(cache_path: Path) -> List[dict]:
    ...
    async with httpx.AsyncClient(timeout=60, follow_redirects=True) as client:
        offset = 0
        batch_size = 100
        max_fetch = 1500  # MedQA test has ~1273 questions

        while offset < max_fetch:
            url = MEDQA5_HF_URL.format(offset=offset, length=batch_size)
            try:
...
async def _download_medqa5_jsonl(cache_path: Path) -> List[dict]:
    """Fallback: download MedQA JSONL and try to use it for 5-option."""
    async with httpx.AsyncClient(timeout=60, follow_redirects=True) as client:
        try:
            r = await client.get(MEDQA5_JSONL_URL)
            r.raise_for_status()
            ...
            print(f"  Cached {len(cases)} MedQA-5 cases to {cache_path}")
            return cases

        except Exception as e:
            print(f"  Warning: Failed to download MedQA-5 JSONL: {e}")
            return []
...
# ──────────────────────────────────────────────
# MCQ answer selection (5-option: A-E)
# ──────────────────────────────────────────────
...
Respond with ONLY the letter (A, B, C, D, or E) on the first line, then a one-sentence justification.
"""
...
async def validate_medqa5(
    ...
    """
    Run MedQA-5 cases through the CDS pipeline and score results.

    Same scoring as MedQA but with 5-option MCQ selection, making
    the random baseline 20% instead of 25%.
    """
        ...
        if report:
            ...
            if include_mcq and case.ground_truth.get("options"):
                try:
                    ...
                    details["n_options"] = case.ground_truth.get("n_options", 5)
                except Exception as e:
    ...
    metrics["random_baseline"] = 0.20  # 1/5 for 5-option
...
async def main():
    """Run MedQA-5 validation standalone."""
    import argparse

    parser = argparse.ArgumentParser(description="MedQA-5 (5-option) Validation")
    ...
    print("MedQA-5 (5-Option) Validation Harness")
    print("=" * 40)

    cases = await fetch_medqa5(max_cases=args.max_cases, seed=args.seed)
